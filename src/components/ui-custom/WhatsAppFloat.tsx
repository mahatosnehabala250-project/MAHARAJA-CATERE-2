    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-8 right-8 z-[999] flex items-end gap-3"
        >
          {/* Tooltip */}
          <AnimatePresence>
            {showTooltip && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
                className="hidden sm:block relative bg-white rounded-lg shadow-xl px-4 py-3 max-w-[200px] border border-green-200"
              >
                {/* Arrow pointing to the button */}
                <div className="absolute right-[-6px] top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-r border-t border-green-200 rotate-[45deg]" />
                <p className="text-sm font-semibold text-gray-800 font-[family-name:var(--font-lato)]">Chat with us!</p>
                <p className="text-xs text-gray-500 mt-1 font-[family-name:var(--font-lato)]">We typically reply within minutes</p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <a
            ...
            className="group relative flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] text-white shadow-2xl hover:shadow-[#25D366]/50 transition-all duration-300 hover:scale-110"
          ...
          >
            ...
            {/* Subtle pulse ring — gentle scale instead of aggressive ping */}
            <span className="absolute inset-0 rounded-full bg-[#25D366] animate-whatsapp-pulse opacity-30" />
          </a>
        </motion.div>
      )}
    </AnimatePresence>
